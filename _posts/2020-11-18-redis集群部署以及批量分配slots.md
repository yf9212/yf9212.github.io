...
### 配置修改
redis安装完之后，复制一份配置文件目录到 /usr/local/etc/中。再复制一份redis-6379.conf

~~~
cp  redis.conf  redis-6379.conf
~~~

修改redis-6379.conf的相关参数：
...
将剩余节点加入进来后，查看集群状态，发现state还是fail，因为16384个槽还未分配。
...
~~~
上面命令只是分配了第0个和第5000个槽给6379这个节点，如果想要范围分配，可以通过redis-cli 执行命令

~~~
redis-cli -p <port> -c  cluster addslots {<start>..<end>}
~~~
如果设置了密码，再增加 -a 参数。注意：范围用 *{}* 包围起来，*start 从0开始，end最大值为16383，中间是2个点，不是有些资料中说的三个点*；
...
查看节点信息，6379端口节点分配到索引号为第0到第5000个槽，6380和6381节点还未分配

下面为6380端口节点分配第5001到第10000的槽
...
查看节点信息，6379端口节点分配索引号第0到第5000个槽，6380端口节点分配索引号第5001到第10000个槽，6381端口未分配。

此时，只剩索引号第10001到16383的槽还未分配，只剩下最后一个节点，将其全部分配在最后一个节点
...
查看节点信息，6379端口节点分配第0到第5000个槽，6380端口节点分配第5001到第10000个槽，6381分配第10001到16383个槽，至此，所有槽已经分配完毕，集群服务状态也发生变更，变为可用。
